from typing import List, Dict, Optional, Type, Callable, Any, Union, Tuple
...
class Competitor:
    ...
    excluded_devices: Optional[List[str]] = None
...
class BenchmarkPlotConfig:
    """Configuration for a single benchmark plot."""
    plot_name: str
    x_arg: str
    x_vals: List[Any]
    init_arg_builder: Callable[[Any], Dict[str, Any]]
    input_provider: Callable[[Dict[str, Any], str], tuple]


@dataclass
class ModuleBenchmarkConfig:
    """Holds all benchmark configurations for a module."""
    competitors: Dict[str, Competitor]
    plots: List[BenchmarkPlotConfig]
...
def discover_dunder_objects(
    ...
    ) -> List[Union[ModuleTestConfig, ModuleBenchmarkConfig]]:
